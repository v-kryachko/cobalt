                                             ...
                                             const egl::AttributeMap &attribs)
{
    ASSERT(configIdToGLXConfig.count(state.config->configID) > 0);
    glx::FBConfig fbConfig = configIdToGLXConfig[state.config->configID];
    return new PixmapSurfaceGLX(state, reinterpret_cast(nativePixmap), mGLX.getDisplay(), mGLX, fbConfig);
}
                                      ...
                                      const egl::AttributeMap &attributes) const
{
    ...
    int status = XGetGeometry(mGLX.getDisplay(), reinterpret_cast(pixmap), &rootWindow, &x, &y, &width, &height,
                              &borderWidth, &depth);
    if (!status)
    {
